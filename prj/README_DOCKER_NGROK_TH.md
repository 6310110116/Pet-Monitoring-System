...
## ใช้ MySQL ที่ติดตั้งบนเครื่อง (ไม่ใช้ db ใน docker)
- ตั้งค่าใน `.env`:
  - `DB_HOST=host.docker.internal`
  - `DB_PORT=3306`
  - `DB_NAME=pet_monitoring`
- รันด้วย `docker compose up -d --build`
- ถ้าต้องการใช้ MySQL ใน docker เดิม ให้ใช้ `docker compose -f docker-compose.localdb.yml up -d --build`
