...
def _uploaded_asset_abspath(val: str) -> Optional[str]:
    """Resolve a DB image value to a local file inside Colorcat/uploads when possible."""
    if val is None:
        return None
    v = str(val).strip()
    if not v:
    ...
    v = v.replace("\\", "/")
    v_low = v.lower()
    if not (v_low.startswith("/assets/uploads/") or "/uploads/" in v_low):
        return None
    filename = _basename_from_any_path(v)
    if not filename:
        return None
...
def _delete_uploaded_asset_file(val: str) -> None:
    path = _uploaded_asset_abspath(val)
    if not path:
        return
    try:
        os.remove(path)
        ...
        print("delete uploaded asset warning:", e)
...
STREAM_PROVIDER = str(os.environ.get('STREAM_PROVIDER', 'opencv')).strip().lower() or 'opencv'
GO2RTC_BASE_URL = str(os.environ.get('GO2RTC_BASE_URL', 'http://go2rtc:1984')).rstrip('/')
GO2RTC_INTERNAL_BASE_URL = str(os.environ.get('GO2RTC_INTERNAL_BASE_URL', 'http://go2rtc:1984')).rstrip('/')
_GO2RTC_LAST_OK_BASE = None
...
def _get_go2rtc_base_candidates():
    candidates = []
    if _is_running_in_docker():
        candidates.append(GO2RTC_INTERNAL_BASE_URL or 'http://go2rtc:1984')
    candidates.append(GO2RTC_BASE_URL or 'http://go2rtc:1984')
    if 'http://go2rtc:1984' not in candidates:
        candidates.append('http://go2rtc:1984')
    # remove empty/duplicate while preserving order
    seen = set()
    ordered = []
    global _GO2RTC_LAST_OK_BASE
    if _GO2RTC_LAST_OK_BASE:
        candidates.insert(0, _GO2RTC_LAST_OK_BASE)
    for c in candidates:
        c = str(c or '').rstrip('/')
...
def _send_base_file_nocache(filename: str):
    return _add_no_cache_headers(send_from_directory(BASE_DIR, filename))
...
APP_TIMEZONE_NAME = (os.environ.get("TZ") or os.environ.get("APP_TIMEZONE") or "Asia/Bangkok").strip() or "Asia/Bangkok"
DB_TIME_ZONE_OFFSET = (os.environ.get("DB_TIME_ZONE_OFFSET") or "+07:00").strip() or "+07:00"
...
def _apply_db_session_settings(conn):
    try:
        cur = conn.cursor()
        try:
            cur.execute("SET time_zone = %s", (DB_TIME_ZONE_OFFSET,))
        finally:
            cur.close()
    except Exception:
        pass
...
def get_db():
    """Create a MySQL connection using db_config and apply session settings."""
    conn = mysql.connector.connect(**db_config)
    return _apply_db_session_settings(conn)


# ============================================================
# LINE Messaging (link per user + push notifications)
# - Link flow (no LIFF):
#   1) user logs in to web and generates a one-time code
#   2) user adds the LINE Official Account (bot) and sends that code in chat
#   3) webhook receives the message and links LINE userId <-> app user_id
# ============================================================
...
LINE_OA_USER_ID = (os.environ.get("LINE_OA_USER_ID") or "").strip()  # optional: @xxxx
...
def _line_verify_signature(raw_body: bytes, signature_b64: str) -> bool:
    """Verify X-Line-Signature."""
    if not signature_b64 or not LINE_CHANNEL_SECRET:
        return False
...
def _line_reply(reply_token: str, text: str):
    if not _line_enabled() or not reply_token:
        return
    try:
        requests.post(
...
def _line_push(line_user_id: str, text: str) -> bool:
    if not _line_enabled() or not line_user_id:
        print("[LINE] not enabled or missing line_user_id")
        return False
    try:
        r = requests.post(
...
def _ensure_line_tables():
    """Ensure line_link_codes + line_links exist."""
    conn = get_db()
    cur = conn.cursor()
    try:
        cur.execute(
...
def _get_state(cursor, k: str, default: str = None) -> str:
    cursor.execute("SELECT v FROM notification_state WHERE k=%s", (k,))
    row = cursor.fetchone()
    if not row:
        return default
    # cursor may be dict or tuple
    if isinstance(row, dict):
        return row.get("v")
    return row[0]


def _set_state(cursor, k: str, v: str):
    cursor.execute(
        """
...
def _format_mysql_dt(dt_val) -> str:
...
def _format_hour_text(hours_val) -> str:
    try:
    ...
    if hours_num <= 0:
        hours_num = 1.0
    if abs(hours_num - round(hours_num)) < 1e-9:
        return f"{int(round(hours_num))} ชั่วโมง"
    txt = f"{hours_num:.2f}".rstrip("0").rstrip(".")
    return f"{txt} ชั่วโมง"


def _build_no_cat_alert_message(cat_name: str, alert_time, no_cat_hours, missing_since=None) -> str:
    cat_txt = str(cat_name or "-").strip() or "-"
    alert_time_txt = _format_mysql_dt(alert_time)
    hours_txt = _format_hour_text(no_cat_hours)

    lines = [
        "🚨 แจ้งเตือนล่าสุด",
        f"แมว: {cat_txt}",
        f"เวลา: {alert_time_txt}",
    ]

    if isinstance(missing_since, datetime):
        lines.append(f"เริ่มไม่พบตั้งแต่: {_format_mysql_dt(missing_since)}")
    elif missing_since not in (None, ""):
        lines.append(f"เริ่มไม่พบตั้งแต่: {_format_mysql_dt(missing_since)}")

    lines.extend([
        "",
        f"ไม่พบ {cat_txt} เกิน {hours_txt}",
    ])
    return "\n".join(lines)

def _line_get_all_linked_user_ids(cursor) -> list[str]:
    cursor.execute("SELECT line_user_id FROM line_links")
    rows = cursor.fetchall() or []
    out = []
    for r in rows:
        if isinstance(r, dict):
            out.append(r.get("line_user_id"))
        else:
            out.append(r[0])
    return [x for x in out if x]


def _line_push_to_all_linked(text: str) -> int:
    if not _line_enabled():
        return 0
    conn = get_db()
    cur = conn.cursor(dictionary=True)
    try:
        line_ids = _line_get_all_linked_user_ids(cur)
    finally:
        try:
            cur.close()
        finally:
...
def _send_web_push_to_all(title: str, body: str, url: str = "/") -> int:
    """Send push to all subscriptions. Returns count of successful sends."""
...
@app.route("/")
def serve_index():
    return _send_base_file_nocache("index.html")
...
@app.route("/script.js")
def serve_script():
    return _send_base_file_nocache("script.js")


@app.route("/style.css")
def serve_style():
    return _send_base_file_nocache("style.css")
...
def serve_login():
    return _send_base_file_nocache("login.html")


@app.route("/auth.js")
def serve_auth_js():
    return _send_base_file_nocache("auth.js")


@app.route("/admin.html")
def serve_admin():
    return _send_base_file_nocache("admin.html")


@app.route("/admin.js")
def serve_admin_js():
    return _send_base_file_nocache("admin.js")


# =========================================
# A) CAMERA CONFIG
# =========================================
# แก้/เพิ่มกล้องได้ที่นี่
# ชื่อภายในหลัง rename:
# - room2  = เดิม Garden
# - hall   = เดิม Garage
# - kitchen = เดิม Kitchen
# - room1  = เดิม Hall
ROOMS_CFG = [
    {
        "name": "room2",
        "cameras": [
            {"label": "Cam1", "rtsp_url": "rtsp://admin:12345678@192.168.43.184:10554/tcp/av0_0"},
        ],
    },
    {
        "name": "hall",
        "cameras": [
            {"label": "Cam3", "rtsp_url": "rtsp://admin:05032544@192.168.43.107:10554/tcp/av0_0"},
        ],
    },
    {
        "name": "kitchen",
        "cameras": [
            {"label": "Cam4", "rtsp_url": "rtsp://admin:12345678@192.168.43.76:10554/tcp/av0_0"},
        ],
    },
    {
        "name": "room1",
        "cameras": [
            {"label": "Cam5", "rtsp_url": "rtsp://admin:12345678@192.168.43.45:10554/tcp/av0_0"},
        ],
    },
]
...
def get_camera_cfg_by_room_index(room_name: str, index: int):
    with _cam_lock:
        room = next((r for r in ROOMS_CFG if r.get("name") == room_name), None)
        if not room:
            return None
...
def _get_stream_name(room_name: str, index: int) -> str:
    return f"{str(room_name or '').strip().lower()}_{int(index)}"


def _candidate_rtsp_urls(rtsp_url: str):
    urls = []
    base = (rtsp_url or '').strip()
    if not base:
        return urls
    urls.append(base)

    m = re.match(r'^(rtsp://[^@]+@[^/]+/)(tcp|udp)(/av0_[01])$', base, flags=re.I)
    if not m:
        return urls

    prefix = m.group(1)
    current_proto = m.group(2).lower()
    current_stream = m.group(3).lower()

    for proto in ('tcp', 'udp'):
        for stream in ('/av0_0', '/av0_1'):
            u = f'{prefix}{proto}{stream}'
            if u not in urls:
                urls.append(u)

    if current_proto == 'tcp' and current_stream == '/av0_0':
        return urls
    return urls


def _read_frame_once(rtsp_url: str, mode: str = 'low', open_timeout_s: float = 3.0):
    cv2 = _get_cv2()
    if cv2 is None:
        return None
    ...
    try:
        ...
        deadline = _time.time() + max(1.0, float(open_timeout_s))
        frame = None
        ...
        frame, quality = _apply_mode(frame, mode)
        return _encode_jpeg(frame, quality)
    except Exception as e:
        print('[camera] direct read failed:', rtsp_url, e, flush=True)
        return None
    finally:
        try:
            if cap is not None:
...
def _proxy_go2rtc_frame(stream_name: str):
    global _GO2RTC_LAST_OK_BASE
    last_error = None
    for base in _get_go2rtc_base_candidates():
        try:
            url = f"{base}/api/frame.jpeg?src={stream_name}&cache=1s"
            r = requests.get(url, timeout=(1.0, 4.0))
            if not r.ok:
                print('[go2rtc] frame proxy failed:', r.status_code, stream_name, 'base=', base, flush=True)
                last_error = f'status={r.status_code}'
                continue
            _GO2RTC_LAST_OK_BASE = base
            return r.content
        except Exception as e:
            last_error = e
            print('[go2rtc] frame proxy error:', stream_name, 'base=', base, e, flush=True)
            continue
    return None


def _iter_go2rtc_mjpeg(stream_name: str):
    global _GO2RTC_LAST_OK_BASE
    last_exc = None
    for base in _get_go2rtc_base_candidates():
        upstream = None
        try:
            url = f"{base}/api/stream.mjpeg?src={stream_name}"
            upstream = requests.get(url, stream=True, timeout=(1.0, 30.0))
            ...
            last_exc = e
            print('[go2rtc] mjpeg proxy error:', stream_name, 'base=', base, e, flush=True)
            continue
        finally:
            try:
...
def _pick_working_rtsp(room_name: str, index: int, mode: str = 'low'):
    cam = get_camera_cfg_by_room_index(room_name, index)
    if not cam:
        return None, None
    rtsp = cam.get('rtsp_url')
    if not rtsp:
        return None, None
    for cand in _candidate_rtsp_urls(rtsp):
        jpg = _read_frame_once(cand, mode=mode, open_timeout_s=2.5)
        if jpg:
            print('[camera] selected source', room_name, index, '->', cand, flush=True)
            return cand, jpg
    return None, None
...
# =========================================
# --- RTSP reader cache (ลดหน่วง/ไม่สะสม backlog) ---
# แนวคิด: แยก thread อ่านกล้อง -> เก็บ "เฟรมล่าสุด" ไว้เสมอ
# จากนั้น MJPEG/Snapshot จะ "ดึงเฟรมล่าสุด" ทำให้ผ่าน ngrok ไม่หน่วงสะสมเป็นสิบวินาที
        ...
        while not self._stop:
            try:
                if cap is None or (hasattr(cap, "isOpened") and not cap.isOpened()):
                    ...
                    # ให้เวลาต่อ RTSP สั้นลงนิดหน่อย (บางกล้องค้างนาน)
                    try:
                ...
                if not ok or frame is None:
                    # ถ้าอ่านไม่ได้ พักก่อนและลองใหม่
                    _time.sleep(0.2)
                    continue
                ...
                # จำกัดการอ่านเฟรมไม่ให้กิน CPU เกินจำเป็น
                _time.sleep(0.01)
    ...
    def get_frame(self):
        with self._lock:
            if self._frame is None:
                return None
            # copy เพื่อกันการถูกแก้ไขระหว่าง encode
    ...
    def is_stale(self, max_age_sec: float = 3.0) -> bool:
        with self._lock:
            if self._last_ok_ts <= 0:
                return True
            return (_time.time() - self._last_ok_ts) > max_age_sec


_rtsp_readers = {}
_rtsp_readers_lock = Lock()


def _get_rtsp_reader(rtsp_url: str) -> _RTSPReader:
    with _rtsp_readers_lock:
        r = _rtsp_readers.get(rtsp_url)
        if r is None:
            r = _RTSPReader(rtsp_url)
            _rtsp_readers[rtsp_url] = r
        return r


def _apply_mode(frame, mode: str):
    """ปรับขนาด/คุณภาพตามโหมด (เหมาะกับ ngrok/มือถือ)"""
    cv2 = _get_cv2()
    if cv2 is None:
        return frame, 80
...
def _encode_jpeg(frame, quality: int = 80):
    ...
    try:
        ret, buf = cv2.imencode(
            ".jpg",
            frame,
            [int(cv2.IMWRITE_JPEG_QUALITY), int(max(10, min(95, quality)))],
        )
        if not ret:
            return None
        return buf.tobytes()
        ...
        return None
def generate_frames_rtsp(rtsp_url: str, mode: str = "high"):
    """อ่านเฟรมจาก RTSP แล้วส่งเป็น MJPEG multipart (ดึงเฟรมล่าสุดจาก reader)

    - ลดการเปิด VideoCapture ซ้ำหลายครั้ง (หนึ่งกล้องหนึ่ง thread)
    - จำกัด fps เพื่อไม่ให้กินแบนด์วิดท์/CPU เกินไป
    - ไม่สะสม backlog (ส่งเฟรมล่าสุดเสมอ)
    """
    if _get_cv2() is None:
        return

    reader = _get_rtsp_reader(rtsp_url)
    ...
    try:
        while True:
            ...
            if frame is None or reader.is_stale(5.0):
                _time.sleep(0.2)
                continue
...
def video_feed_room_index(room_name, index):
    cam_cfg = get_camera_cfg_by_room_index(room_name, index)
    if not cam_cfg:
        return Response("camera not found", status=404)

    mode = request.args.get('mode', 'high')
    stream_name = _get_stream_name(room_name, index)

    if STREAM_PROVIDER == 'go2rtc':
        try:
            resp = Response(
                _iter_go2rtc_mjpeg(stream_name),
    ...
    rtsp, _ = _pick_working_rtsp(room_name, index, mode='low')
    if not rtsp:
        return Response('no working camera source', status=503, mimetype='text/plain')

    resp = Response(
        generate_frames_rtsp(rtsp, mode=mode),
        mimetype="multipart/x-mixed-replace; boundary=frame",
    )
    resp.headers["Cache-Control"] = "no-store, no-cache, must-revalidate, max-age=0"
    resp.headers["Pragma"] = "no-cache"
...
# Snapshot endpoint (เหมาะกับ ngrok/มือถือ: ไม่หน่วงสะสม)
@app.route("/snapshot/<room_name>/<int:index>")
def snapshot_room_index(room_name, index):
    cam_cfg = get_camera_cfg_by_room_index(room_name, index)
    if not cam_cfg:
        return Response("camera not found", status=404)
    ...
    if STREAM_PROVIDER == 'go2rtc':
        jpg = _proxy_go2rtc_frame(stream_name)
        if jpg:
    ...
    rtsp, jpg = _pick_working_rtsp(room_name, index, mode=mode)
    if not rtsp or not jpg:
...
@app.route("/api/stream_sources/<room_name>/<int:index>", methods=["GET"])
def api_stream_sources(room_name, index):
    cam = get_camera_cfg_by_room_index(room_name, index)
    if not cam:
        return jsonify({"error": "camera not found"}), 404

    stream_name = _get_stream_name(room_name, index)
    snapshot_url = f"/snapshot/{room_name}/{index}?mode=low"
    mjpeg_url = f"/video_feed/{room_name}/{index}?mode=high"
    payload = {
        "room": room_name,
        "index": index,
        "label": cam.get("label", f"Camera {index+1}"),
        "stream_name": stream_name,
        "snapshot_url": snapshot_url,
        "snapshotUrl": snapshot_url,
        "mjpeg_url": mjpeg_url,
        "mjpegUrl": mjpeg_url,
        "video_feed": mjpeg_url,
        "videoFeed": mjpeg_url,
        "hls_url": None,
        "hlsUrl": None,
        "webrtc_url": None,
...
def _resolve_cat_row(cursor, cat_name: str = "", cat_color: str = "") -> Optional[dict]:
    """Resolve a cat by name or color and return its canonical DB row."""
    cat_name = str(cat_name or "").strip()
    cat_color = str(cat_color or "").strip()

    if cat_name:
        cursor.execute(
            "SELECT name, image_url, real_image_url, color, display_status FROM cats WHERE TRIM(name)=%s LIMIT 1",
            (cat_name,),
        )
    ...
    if cat_color:
        cursor.execute(
            "SELECT name, image_url, real_image_url, color, display_status FROM cats WHERE TRIM(color)=%s LIMIT 1",
            (cat_color,),
        )
        row = cursor.fetchone() or {}
        if row:
...
def _timeslot_datetime_expr(cols: set) -> str | None:
    """Return SQL expression that represents the timeslot datetime.
    Supports both schemas:
      - legacy: timeslot.date_slot (DATETIME)
      - new: timeslot.date (DATE) + timeslot.slot (TIME)
    """
    if "date_slot" in cols:
        return "`date_slot`"
    if "date" in cols and "slot" in cols:
        return "TIMESTAMP(`date`, `slot`)"
    return None


def _timeslot_date_expr(cols: set) -> str | None:
    """Return SQL expression for the date part of a timeslot record."""
    if "date" in cols:
        return "`date`"
    if "date_slot" in cols:
        return "DATE(`date_slot`)"
    return None
...
def _get_available_timeslot_prefixes(cursor) -> list[str]:
    """คืนรายชื่อ prefix สี/แมวที่มีคอลัมน์ครบในตาราง timeslot จริง

    เงื่อนไขว่าพร้อมใช้งานสำหรับ statistics/timeline คือ ต้องมีอย่างน้อย
      - prefix
      - prefix_cam
      - prefix_ac
    """
    ...
    for col in cols:
        if not isinstance(col, str):
            continue
        if not col.endswith("_ac"):
            continue

        prefix = col[:-3]
        if not _safe_identifier(prefix):
            continue

        if prefix in cols and f"{prefix}_cam" in cols and f"{prefix}_ac" in cols:
            prefixes.add(prefix)
...
def _timeslot_get_latest_for_cat(cursor, prefix: str):
    ...
    cols = _get_timeslot_columns(cursor)
    dt_expr = _timeslot_datetime_expr(cols)
    if not dt_expr:
        return None
...
def _timeslot_get_last_found_time(cursor, prefix: str):
    ...
    cols = _get_timeslot_columns(cursor)
    dt_expr = _timeslot_datetime_expr(cols)
    if not dt_expr or status_col not in cols:
        return None
    if not _safe_identifier(status_col):
        return None
    sql = f"""
        SELECT MAX({dt_expr}) AS last_found
        FROM timeslot
        WHERE `{status_col}` = 'F'
    """
    cursor.execute(sql)
...
def _fetch_timeslots_for_cat(cursor, prefix: str, start_dt: datetime, end_dt: datetime):
    ...
    รองรับทั้ง schema แบบเดิม (date_slot) และ schema ใหม่ (date + slot)
    """
    status_col = prefix
    cam_col = f"{prefix}_cam"
    ac_col = f"{prefix}_ac"
    ...
    if not ({status_col, cam_col, ac_col}.issubset(cols)):
        return []
...
def _count_activity_transitions(slots, target_activity: str):
    """นับ "จำนวนครั้ง" แบบ session (ไม่ยึด F/NF)

    แนวคิด:
      - ยึดจากคอลัมน์ *_ac โดยตรง (activity)
      - รวมเหตุการณ์ที่เกิดติดกันภายใน X นาทีเป็น 1 ครั้ง (session)

    ปรับ X ได้ผ่าน env: ACTIVITY_SESSION_GAP_MINUTES (default 30)
    """
    try:
        gap_minutes = int(os.environ.get("ACTIVITY_SESSION_GAP_MINUTES", "30"))
    except Exception:
        gap_minutes = 30
    ...
    for s in slots:
        ...
        try:
            ...
            # ถ้า format เวลาแปลก ให้เริ่มนับ session ใหม่
...
def _latest_date_in_timeslot(cursor):
    cols = _get_timeslot_columns(cursor)
    d_expr = _timeslot_date_expr(cols)
    if not d_expr:
        return None
    cursor.execute(f"SELECT MAX({d_expr}) AS d FROM timeslot")
    row = cursor.fetchone() or {}
    return row.get("d")
...
def _latest_datetime_in_timeslot(cursor):
    ...
    if not dt_expr:
        return None
    cursor.execute(f"SELECT MAX({dt_expr}) AS dt FROM timeslot")
    row = cursor.fetchone() or {}
    return row.get("dt")
...
def _latest_datetime_in_timeslot_day(cursor, day_start: datetime, day_end: datetime):
    ...
    if not dt_expr:
        return None
    cursor.execute(
        f"SELECT MAX({dt_expr}) AS dt FROM timeslot WHERE {dt_expr} >= %s AND {dt_expr} < %s",
        (day_start, day_end),
    )
    row = cursor.fetchone() or {}
    return row.get("dt")
...
def _get_alert_target_day(cursor) -> date:
    ...
    return d if d else _today_local()
...
def get_system_config():
    ...
    รองรับหลายแมวโดยอิงจากสี (cats.color):
      - ถ้า query ?cat=<ชื่อแมว> หรือ ?catColor=<สี> จะคืนค่า config เฉพาะแมวตัวนั้น
      - ถ้าไม่ส่ง cat/catColor จะคืนค่า global active เหมือนเดิม

    Response เป็น camelCase และมี field เพิ่ม:
      - scope: "global" | "cat"
      - catName, catColor (เฉพาะ scope=cat)
    """
    cat_name = (request.args.get("cat") or request.args.get("catName") or "").strip()
    cat_color = (request.args.get("catColor") or request.args.get("color") or "").strip()

    conn = get_db()
    cur = conn.cursor(dictionary=True)
    try:
        if not cat_name and not cat_color:
            cfg = apply_config_cursor(cur, ACTIVE_CONFIG_ID)
            if not cfg:
                return _add_no_cache_headers(jsonify({"message": "Error fetching system config"})), 500
            cfg["scope"] = "global"
            return _add_no_cache_headers(jsonify(cfg))

        crow = _resolve_cat_row(cur, cat_name=cat_name, cat_color=cat_color)
        if not crow:
            ident = cat_name or cat_color
            return _add_no_cache_headers(jsonify({"message": f"cat not found: {ident}"})), 404

        cat_name = (crow.get("name") or "").strip()
...
def update_system_config():
    ...
    Body (camelCase):
      ...
      - catName/cat (optional), catColor/color (optional)
    """
    body = request.get_json(silent=True) or {}
    cat_name = str(body.get("catName") or body.get("cat") or "").strip()
    cat_color = str(body.get("catColor") or body.get("color") or "").strip()

    conn = get_db()
    cur = conn.cursor(dictionary=True)
    try:
        if cat_name or cat_color:
            crow = _resolve_cat_row(cur, cat_name=cat_name, cat_color=cat_color)
            if not crow:
                ident = cat_name or cat_color
                return jsonify({"message": f"cat not found: {ident}"}), 404
            cat_name = (crow.get("name") or cat_name).strip()
            cat_color = (crow.get("color") or cat_color).strip()

        def _validate_and_merge(base_cfg: dict):
            merged = {}
            for snake_key, camel_key in SNAKE_TO_CAMEL.items():
                raw = body.get(camel_key, base_cfg.get(snake_key))
                if raw is None:
                    merged[snake_key] = None
                    continue
                if isinstance(raw, str):
                    raw = raw.strip()
                    if raw == "":
                        return None, (jsonify({"message": f"{camel_key} ต้องเป็นตัวเลข"}), 400)
                try:
                    merged[snake_key] = int(raw)
                except (TypeError, ValueError):
                    return None, (jsonify({"message": f"{camel_key} ต้องเป็นตัวเลข"}), 400)
                if merged[snake_key] < 0:
                    return None, (jsonify({"message": f"{camel_key} ต้องมากกว่าหรือเท่ากับ 0"}), 400)

            min_ex = merged.get("alert_no_excrete_min")
            max_ex = merged.get("alert_no_excrete_max")
            if min_ex is not None and max_ex is not None and min_ex > max_ex:
                return None, (jsonify({"message": "minExcretion ต้องน้อยกว่าหรือเท่ากับ maxExcretion"}), 400)
            return merged, None
        ...
        if cat_color:
            ...
            merged, err = _validate_and_merge(current_snake)
            if err:
                return err
            ...
            effective_cfg = row_to_camel(_get_system_config_for_cat(cur, cat_color) or current_snake)
            effective_cfg.pop("updated_at", None)
            effective_cfg["scope"] = "cat"
            effective_cfg["catColor"] = cat_color
            effective_cfg["catName"] = cat_name
            return jsonify({"message": "Config updated successfully", "scope": "cat", "catColor": cat_color, "config": effective_cfg})
        ...
        merged, err = _validate_and_merge(current_cfg_snake)
        if err:
            return err
        ...
        effective_cfg = row_to_camel(_get_system_config_global(cur) or current_cfg_snake)
        effective_cfg["scope"] = "global"
        return jsonify({"message": "Config updated successfully", "scope": "global", "config": effective_cfg})
    except mysql.connector.Error as e:
        conn.rollback()
        print("update_system_config mysql error:", e)
        return jsonify({"message": f"Database error: {e}"}), 500
    except Exception as e:
        conn.rollback()
        print("update_system_config unexpected error:", e)
...
def get_system_config_summaries():
    """คืนค่าสรุปของระบบต่อแมวที่มีค่าเดิมซ้ำกัน >= 3 เดือน"""
    conn = get_db()
    cur = conn.cursor(dictionary=True)
    try:
        cat_q = (request.args.get("cat") or request.args.get("catName") or "").strip()

        filters = ["c.display_status = 1"]
        params = []

        if cat_q:
            cur.execute("SELECT color FROM cats WHERE TRIM(name)=%s LIMIT 1", (cat_q,))
            row = cur.fetchone() or {}
        ...
        sql = f"""
            ...
            JOIN cats c ON c.color = m.cat_color
            {where_clause}
            GROUP BY
              m.cat_color,
        ...
        for r in rows:
            ...
            if not color or color in chosen:
                continue
            chosen[color] = {
        ...
        return _add_no_cache_headers(jsonify(result))
    except mysql.connector.Error as e:
        print("get_system_config_summaries mysql error:", e)
        return _add_no_cache_headers(jsonify({"message": f"Database error: {e}"})), 500
    finally:
        cur.close()
        conn.close()
...
def apply_system_config_summary():
    """บันทึกค่าสรุปลง system_config_cat เฉพาะแมว"""
    body = request.get_json(silent=True) or {}
    cat_color = (body.get("catColor") or "").strip()
    cat_name = (body.get("catName") or body.get("cat") or "").strip()
    ...
    try:
        ...
        conn.commit()
        return jsonify({"message": "Applied summary config", "scope": "cat", "catColor": cat_color, "catName": cat_name})
    except mysql.connector.Error as e:
        conn.rollback()
        print("apply_system_config_summary mysql error:", e)
        return jsonify({"message": f"Database error: {e}"}), 500
    finally:
        cur.close()
        conn.close()
...
def reset_system_config():
    """รีเซ็ต System Config ทั้ง global หรือเฉพาะแมว"""
    cat_name = (request.args.get("cat") or request.args.get("catName") or "").strip()
    cat_color = (request.args.get("catColor") or request.args.get("color") or "").strip()
    ...
    try:
        if cat_name or cat_color:
            ...
            cur.execute("DELETE FROM system_config_cat WHERE cat_color=%s", (cat_color,))
            conn.commit()
            return jsonify({"message": "Cat config has been reset to global defaults", "scope": "cat", "catColor": cat_color, "catName": cat_name})
        ...
        conn.rollback()
        print("reset_system_config mysql error:", e)
        return jsonify({"message": f"Database error: {e}"}), 500
    finally:
        cur.close()
        conn.close()
...
def monthly_rollup():
    ...
    """
    body = request.get_json(silent=True) or {}
    month_ym = body.get("month_ym") or _prev_month_ym(_today_local())
...
def _time_first_missing_since(cursor, prefix: str, last_found=None):
    """เวลาเริ่มต้นของช่วงที่ไม่พบแมวต่อเนื่องล่าสุด (status='NF').

    ถ้าเคยมี F มาก่อน จะหา NF ตัวแรกหลังจาก F ล่าสุด
    ถ้ายังไม่เคยมี F เลย จะใช้ NF ตัวแรกทั้งหมด
    """
    cols = _get_timeslot_columns(cursor)
    dt_expr = _timeslot_datetime_expr(cols)
    status_col = prefix
    if not dt_expr or status_col not in cols or not _safe_identifier(status_col):
        return None
    sql = f"SELECT MIN({dt_expr}) AS first_missing FROM timeslot WHERE `{status_col}` = 'NF'"
    params = []
    if last_found is not None:
        sql += f" AND {dt_expr} > %s"
        params.append(last_found)
    cursor.execute(sql, tuple(params))
    row = cursor.fetchone() or {}
    return row.get("first_missing")
...
def _compute_alerts_for_day(cursor, target_day: date):
    ...
    for cat_name, prefix in cat_prefix.items():
        ...
        missing_since = _time_first_missing_since(cursor, prefix, last_found)
        reference_dt = missing_since or last_found
        if reference_dt:
            hours_since = (ref_dt - reference_dt).total_seconds() / 3600.0
            if hours_since >= no_cat_hours:
                alerts.append(
                    {
                        "cat_name": cat_name,
                        "alert_type": "no_cat",
                        "message": _build_no_cat_alert_message(cat_name, ref_dt, no_cat_hours, reference_dt),
                        "created_at": ref_dt,
                        "missing_since": reference_dt,
                        "last_found": reference_dt,
                        "repeat_hours": no_cat_hours,
                    }
                )
...
def _ensure_alerts_log_schema():
    """Normalize alerts_log indexes for repeatable no_cat alerts and worker startup."""
    conn = get_db()
    cur = conn.cursor(dictionary=True)
    try:
        cur.execute(
        ...
        indexes = {str(r.get("INDEX_NAME") or ""): int(r.get("NON_UNIQUE") or 0) for r in (cur.fetchall() or [])}

        if indexes.get("uniq_daily") == 0:
            cur.execute("ALTER TABLE alerts_log DROP INDEX uniq_daily")

        if "idx_alerts_log_daily_lookup" not in indexes:
            cur.execute(
                "CREATE INDEX idx_alerts_log_daily_lookup ON alerts_log (cat_name, alert_type, alert_date, is_read)"
            )
        if "idx_alerts_log_repeat_lookup" not in indexes:
            cur.execute(
                "CREATE INDEX idx_alerts_log_repeat_lookup ON alerts_log (cat_name, alert_type, created_at, is_read)"
            )
...
def _is_month_complete_in_timeslot(cursor, month_start: datetime, month_end: datetime, days_in_month: int) -> bool:
    ...
    (นับจาก DISTINCT วันที่ในช่วง [start, end))
    รองรับทั้ง schema แบบเดิม (date_slot) และ schema ใหม่ (date + slot)
    """
    cols = _get_timeslot_columns(cursor)
    dt_expr = _timeslot_datetime_expr(cols)
    d_expr = _timeslot_date_expr(cols)
    if not dt_expr or not d_expr:
        return False

    cursor.execute(
        f"""
        SELECT COUNT(DISTINCT {d_expr}) AS dcnt
        FROM timeslot
        WHERE {dt_expr} >= %s AND {dt_expr} < %s
        """,
...
def _maybe_monthly_rollup(cursor):
    ...
    month_ym = _prev_month_ym(_today_local())
...
def list_alerts():
    ...
    try:
        ...
        inserted = 0
        # read-only by default; ingest only when explicitly requested with ingest=1
        mode = (request.args.get("mode") or "").strip().lower()
        date_str = (request.args.get("date") or "").strip()
        should_ingest = (request.args.get("ingest") or "0").strip().lower() in ("1", "true", "yes", "y", "on")
        ...
        # If new alerts were inserted, send a push notification (optional)
        try:
            if inserted and inserted > 0:
                _send_web_push_to_all(
                    title='Pet Monitoring',
                    body=f'มีการแจ้งเตือนใหม่ {inserted} รายการ',
                    url='/',
                )
        except Exception:
            pass
        ...
        for r in cats_rows:
            ...
            if prefix:
                ...
                if _safe_identifier(status_col) and _safe_identifier(cam_col):
                    cols = _get_timeslot_columns(cursor)
                    dt_expr = _timeslot_datetime_expr(cols)
                    if dt_expr and status_col in cols and cam_col in cols:
                        sql = f"""
...
def update_cat():
    ...
    conn = get_db()
    cur = conn.cursor(dictionary=True)
    old_uploaded_to_delete = None
    effective_name = old_name
    try:
        cat_row = _resolve_cat_row(cur, cat_name=old_name)
        if not cat_row:
            return jsonify({"error": f"cat not found: {old_name}"}), 404

        effective_name = (cat_row.get("name") or old_name).strip()
        current_real = normalize_image_to_url(cat_row.get("real_image_url"))

        if new_name and new_name != effective_name:
            dup = _resolve_cat_row(cur, cat_name=new_name)
            if dup:
                return jsonify({"error": f"cat name already exists: {new_name}"}), 409

            cur.execute("UPDATE cats SET name=%s WHERE name=%s", (new_name, effective_name))
            if int(cur.rowcount or 0) < 1:
                return jsonify({"error": f"cat not found: {effective_name}"}), 404

            cascade_sql = [
                ("UPDATE alerts_log SET cat_name=%s WHERE cat_name=%s", (new_name, effective_name)),
                ("UPDATE alerts_log SET message=REPLACE(message, %s, %s) WHERE cat_name=%s", (effective_name, new_name, new_name)),
                ("UPDATE cat_activities SET cat_name=%s WHERE cat_name=%s", (new_name, effective_name)),
                ("UPDATE cat_movements SET cat_name=%s WHERE cat_name=%s", (new_name, effective_name)),
                ("UPDATE cat_config_monthly SET cat_name=%s WHERE cat_name=%s", (new_name, effective_name)),
            ]
            for sql, params in cascade_sql:
                try:
                    cur.execute(sql, params)
                except mysql.connector.Error as e:
        ...
        if reset_image:
            cur.execute("UPDATE cats SET real_image_url=NULL WHERE name=%s", (effective_name,))
            old_uploaded_to_delete = current_real
        elif isinstance(new_real, str) and new_real.strip():
            cur.execute("UPDATE cats SET real_image_url=%s WHERE name=%s", (new_real.strip(), effective_name))
            if current_real and current_real != new_real:
                old_uploaded_to_delete = current_real

        conn.commit()

        cur.execute(
            "SELECT name, image_url, real_image_url, color, display_status FROM cats WHERE name=%s LIMIT 1",
            (effective_name,),
        )
        updated_cat = cur.fetchone() or {}
    except mysql.connector.Error as e:
        conn.rollback()
        print("update_cat mysql error:", e)
        return jsonify({"error": f"Database error: {e}"}), 500
    finally:
        cur.close()
        conn.close()

    if old_uploaded_to_delete and old_uploaded_to_delete != new_real:
        _delete_uploaded_asset_file(old_uploaded_to_delete)

    return jsonify({
        "message": "updated",
        "cat": {
            "name": updated_cat.get("name"),
            "image_url": normalize_image_to_url(updated_cat.get("image_url")),
            "real_image_url": normalize_image_to_url(updated_cat.get("real_image_url")),
            "color": updated_cat.get("color"),
            "display_status": updated_cat.get("display_status"),
        },
    }), 200
...
def upload_cat_image():
    ...
    cur = conn.cursor(dictionary=True)
    try:
        cat_row = _resolve_cat_row(cur, cat_name=cat_name)
        if not cat_row:
            try:
                os.remove(save_path)
            except Exception:
                pass
            return jsonify({"error": f"cat not found: {cat_name}"}), 404

        canonical_name = (cat_row.get("name") or cat_name).strip()
        current_real = normalize_image_to_url(cat_row.get("real_image_url"))

        cur.execute("UPDATE cats SET real_image_url=%s WHERE name=%s", (url_path, canonical_name))
        if int(cur.rowcount or 0) < 1:
            raise mysql.connector.Error(msg=f"failed to update cat image for {canonical_name}")

        conn.commit()

        if current_real and current_real != url_path:
            old_uploaded_to_delete = current_real
        ...
        try:
            ...
            pass
        print("upload_cat_image mysql error:", e)
        return jsonify({"error": f"Database error: {e}"}), 500
    finally:
        cur.close()
        conn.close()

    if old_uploaded_to_delete:
        _delete_uploaded_asset_file(old_uploaded_to_delete)

    return jsonify({
        "ok": True,
        "url": url_path,
        "real_image_url": url_path,
        "cat": {
            "name": updated_cat.get("name"),
            "image_url": normalize_image_to_url(updated_cat.get("image_url")),
            "real_image_url": normalize_image_to_url(updated_cat.get("real_image_url")),
...
def get_timeline_timeslot():
    ...
    try:
        ...
        dt_expr = _timeslot_datetime_expr(cols)
        if not dt_expr:
            return jsonify({"message": "timeslot schema missing datetime columns"}), 500

        where = [f"{dt_expr} >= %s", f"{dt_expr} < %s"]
        params = [day_start, day_end]

        if before_dt:
            where.append(f"{dt_expr} < %s")
            params.append(before_dt)

        # ดึงมากกว่า 1 แถวเพื่อเช็ค has_more
        sql = f"""
            SELECT
              {dt_expr} AS date_slot,
              `{prefix}` AS status,
              `{prefix}_cam` AS cam,
...
def api_timeline_table():
    ...
    try:
        ...
        if not dt_expr:
            ...
            return jsonify({"date": date_str, "hours": [f"{h:02d}" for h in range(24)], "rows": []})
        hours = [f"{h:02d}" for h in range(24)]
        rows = []

        for cat in cats:
            prefix = _normalize_prefix(cat.get("color") or cat.get("name") or "")
            if not prefix:
                continue
...
@app.route("/api/statistics/prefixes", methods=["GET"])
def api_statistics_prefixes():
    """คืน prefix สีที่มีคอลัมน์ครบอยู่จริงในตาราง timeslot"""
    connection = mysql.connector.connect(**db_config)
    cursor = connection.cursor(dictionary=True)
    try:
        prefixes = _get_available_timeslot_prefixes(cursor)
...
def api_statistics_years():
    """คืน 'ทุกปี' ที่มีข้อมูลใน timeslot (เรียง ASC) — รองรับทั้ง schema"""
    connection = mysql.connector.connect(**db_config)
    cursor = connection.cursor(dictionary=True)
    try:
        cols = _get_timeslot_columns(cursor)
        ...
        cursor.execute(
            f"""
            SELECT DISTINCT YEAR({dt_expr}) AS y
            FROM timeslot
...
def api_statistics():
    ...
    try:
        ...
        cursor.execute(
            f"""
            SELECT MIN(YEAR({dt_expr})) AS miny,
                   MAX(YEAR({dt_expr})) AS maxy
            FROM timeslot
            WHERE {dt_expr} IS NOT NULL
            """
        )
...
def _ingest_alerts_list(cursor, target_day: date, alerts: list[dict]) -> int:
    """Insert alerts list into alerts_log.
    ...
    alerts item schema:
      {
        ...
        "alert_type": str,
        "message": str,
        # optional for realtime no_cat:
        "created_at": datetime,
        "last_found": datetime,
        "repeat_hours": int | float,
      }

    Daily alerts (eating/excretion) are still de-duplicated per day.
    Realtime no_cat alerts are de-duplicated by interval, so if the threshold is
    1 hour the system can notify again every 1 hour while the cat is still missing.
    ...
    for al in alerts:
        ...
        custom_created_at = al.get("created_at")
        if isinstance(custom_created_at, datetime):
            created_at = custom_created_at
        else:
            created_at = datetime.combine(target_day, time(23, 59, 0))

        if alert_type == "no_cat":
            repeat_hours = al.get("repeat_hours")
            try:
                repeat_hours = max(float(repeat_hours), 1.0)
            except Exception:
                repeat_hours = 1.0
            repeat_delta = timedelta(hours=repeat_hours)
            missing_since = al.get("missing_since") or al.get("last_found")

            cursor.execute(
                f"""
                SELECT id, created_at
                FROM `{table}`
                WHERE cat_name = %s AND alert_type = %s AND is_read <> 2
            ...
            prev_created_at = prev.get("created_at")

            # รองรับข้อมูลเก่าที่เคยบันทึก realtime เป็นเวลา 23:59 ของวันนั้น
            # ซึ่งทำให้ created_at กลายเป็น "อนาคต" เมื่อเทียบกับเวลารันจริง
            if isinstance(prev_created_at, datetime) and prev_created_at > created_at + timedelta(minutes=5):
                prev_created_at = None

            # ครั้งแรก: ต้องครบ threshold จากเวลาเริ่มหายจริงก่อน
            if isinstance(missing_since, datetime):
                first_trigger_at = missing_since + repeat_delta
                if created_at < first_trigger_at:
                    continue

            # ครั้งถัดไป: ต้องห่างจาก alert ก่อนหน้าอย่างน้อย threshold เต็ม
            if isinstance(prev_created_at, datetime) and created_at - prev_created_at < repeat_delta:
                continue
        else:
            # Daily alerts กันซ้ำต่อวันเหมือนเดิม
            cursor.execute(
                f"""
                SELECT id
                FROM `{table}`
        ...
        if alert_date_generated:
            ...
            if has_color:
                cursor.execute(
                    ...
                    VALUES (%s, %s, %s, %s, 0, %s, %s)
                    """,
                    (cat_name, color_val, alert_type, message, target_day, created_at),
                )
            else:
                cursor.execute(
                    f"""
                    ...
                    VALUES (%s, %s, %s, 0, %s, %s)
                    """,
                    (cat_name, alert_type, message, target_day, created_at),
                )
...
def _compute_no_cat_realtime(cursor) -> list[dict]:
    ...
    now_dt = _now_local().replace(tzinfo=None)
    ...
    for cat_name, prefix in cat_prefix.items():
        ...
        last_found = _time_last_found(cursor, prefix)
        missing_since = _time_first_missing_since(cursor, prefix, last_found)
        if last_found is None and missing_since is None:
            continue
        ...
        hours_since = (now_dt - reference_dt).total_seconds() / 3600.0
        if hours_since >= no_cat_hours:
            alerts.append(
                {
                    "cat_name": cat_name,
                    "alert_type": "no_cat",
                    "message": _build_no_cat_alert_message(cat_name, now_dt, no_cat_hours, reference_dt),
                    "created_at": now_dt,
                    "missing_since": reference_dt,
                    "last_found": reference_dt,
                    "repeat_hours": no_cat_hours,
...
# ============================================================
# LINE Link API (for logged-in users)
# ============================================================
...
def line_status():
    ...
    cur = conn.cursor(dictionary=True)
    try:
        cur.execute("SELECT line_user_id, created_at FROM line_links WHERE user_id=%s", (session["user_id"],))
        row = cur.fetchone()
        return jsonify(
            {
                "ok": True,
                "enabled": _line_enabled(),
                "linked": bool(row),
                "line_user_id": (row or {}).get("line_user_id"),
                "linked_at": (row or {}).get("created_at"),
                "oa_user_id": LINE_OA_USER_ID or None,
            }
        )
    finally:
        try:
...
@app.get("/api/line/link-code")
def line_link_code():
    """Generate a short-lived code for linking the user's LINE account."""
    err = _require_login()
    if err:
        return err

    if not _line_enabled():
        return jsonify({"ok": False, "error": "line_not_configured"}), 503
    ...
    cur = conn.cursor()
    try:
        # remove previous non-expired codes for this user (best-effort)
...
def line_unlink():
    ...
    cur = conn.cursor()
    try:
        cur.execute("DELETE FROM line_links WHERE user_id=%s", (session["user_id"],))
        conn.commit()
        return jsonify({"ok": True})
    finally:
        try:
...
def line_test_push():
    """Trigger realtime alert ingestion and push the latest unread alert to the linked LINE user."""
    err = _require_login()
    if err:
        return err
    ...
    cur = conn.cursor(dictionary=True)
    try:
        # 1) LINE ของ user ที่ login อยู่
        cur.execute("SELECT line_user_id FROM line_links WHERE user_id=%s", (session["user_id"],))
        row = cur.fetchone() or {}
        lid = row.get("line_user_id")
        if not lid:
            return jsonify({"ok": False, "error": "not_linked"}), 400

        # 2) optional trigger ingest (default = read-only)
        inserted = 0
        should_ingest = (request.args.get("ingest") or "0").strip().lower() in ("1", "true", "yes", "y", "on")
        if should_ingest:
            try:
        ...
        # 3) ดึง “alert ล่าสุดที่ยัง unread”
        cur.execute(
            """
            SELECT id, cat_name AS cat, message, created_at
            FROM alerts_log
            WHERE is_read=0 AND is_read <> 2
            ORDER BY created_at DESC, id DESC
            LIMIT 1
            """
        )
        ...
        # 4) ถ้าไม่มี alert ให้ส่งข้อความบอกแทน
        if not al:
            ok = _line_push(lid, "✅ ทดสอบส่ง LINE: ตอนนี้ยังไม่มีแจ้งเตือนที่เข้าเงื่อนไขระบบ")
            return jsonify({"ok": True, "sent": bool(ok), "inserted": int(inserted), "has_alert": False})

        msg = (al.get("message") or "").strip()
        cat = (al.get("cat") or "-")
        ...
        if msg.startswith("🚨"):
            ...
            text = f"🚨 แจ้งเตือนล่าสุด\nแมว: {cat}\nเวลา: {_format_mysql_dt(created_at)}\n\n{msg}"
        ok = _line_push(lid, text)

        return jsonify(
            {"ok": True, "sent": bool(ok), "inserted": int(inserted), "has_alert": True, "alert_id": al.get("id")}
        )
    finally:
        try:
            cur.close()
...
# - Configure webhook URL to: <BASE_URL>/api/line/webhook
# ============================================================
...
def line_webhook():
    ...
    if not _line_verify_signature(raw, sig):
        return "bad signature", 401

    payload = request.get_json(silent=True) or {}
    events = payload.get("events") or []

    # Use a DB connection per webhook for atomic link
    conn = get_db()
    cur = conn.cursor(dictionary=True)
    try:
        now_dt = _now_local().replace(tzinfo=None)
        ...
        for ev in events:
            ...
            if ev_type == "follow":
                _line_reply(reply_token, "สวัสดีค่ะ ✅\nถ้าต้องการเชื่อมต่อกับบัญชีในเว็บ ให้เข้าไปที่หน้า Notification Settings แล้วกด 'เชื่อมต่อ LINE' เพื่อรับรหัส และส่งรหัสนั้นมาที่แชทนี้ค่ะ")
                continue
            ...
            if msg.get("type") != "text":
                _line_reply(reply_token, "กรุณาส่งเป็นข้อความตัวอักษรเท่านั้นค่ะ")
                continue
            ...
            if not code_row:
                _line_reply(reply_token, "ไม่พบรหัสเชื่อมต่อค่ะ\nกรุณากดสร้างรหัสจากในเว็บใหม่ (รหัสหมดอายุภายใน 10 นาที)")
                continue
            ...
            if expires_at and expires_at < now_dt:
                # expired
                cur.execute("DELETE FROM line_link_codes WHERE id=%s", (code_row["id"],))
                conn.commit()
                _line_reply(reply_token, "รหัสหมดอายุแล้วค่ะ\nกรุณากดสร้างรหัสจากในเว็บใหม่")
                continue

            if not line_user_id:
                _line_reply(reply_token, "ไม่สามารถอ่าน LINE userId ได้ค่ะ (source.userId ว่าง)")
                continue

            user_id = int(code_row["user_id"])

            # Link (upsert by user_id). If this LINE userId is already linked to another user, block.
            cur.execute("SELECT user_id FROM line_links WHERE line_user_id=%s", (line_user_id,))
            existed = cur.fetchone()
            if existed and int(existed.get("user_id") or 0) != user_id:
                _line_reply(reply_token, "LINE นี้ถูกเชื่อมต่อกับบัญชีอื่นแล้วค่ะ\nถ้าต้องการย้ายบัญชี ให้ยกเลิกการเชื่อมต่อจากบัญชีเดิมก่อน")
                continue

            cur.execute(
                """
                INSERT INTO line_links (user_id, line_user_id)
                VALUES (%s, %s)
            ...
            cur.execute("DELETE FROM line_link_codes WHERE id=%s", (code_row["id"],))
            conn.commit()
            _line_reply(reply_token, "✅ เชื่อมต่อสำเร็จ!\nต่อไปนี้ระบบจะส่งการแจ้งเตือนมาที่ LINE นี้ค่ะ")
...
def _run_daily_summary_job():
    """Run daily behavior alerts for the latest day that actually has data."""
    connection = mysql.connector.connect(**db_config)
    cursor = connection.cursor(dictionary=True)
    try:
        target_day = _get_alert_target_day(cursor)
...
def _run_line_alert_push_job():
    """Poll new rows in alerts_log and push only *recent* alerts to LINE.

    This prevents replaying historical alerts after restart, state reset, or
    when old alerts are backfilled into alerts_log. The recent window defaults
    to 10 minutes and can be changed with LINE_ALERT_MAX_AGE_MINUTES.
    """
    if not _line_enabled():
        return
    conn = get_db()
    ...
    try:
        _ensure_notification_state_table()
        last_id_s = _get_state(cur, "last_alert_push_id", default="0") or "0"
        try:
            last_id = int(last_id_s)
        except Exception:
            last_id = 0

        try:
            max_age_minutes = int(os.environ.get("LINE_ALERT_MAX_AGE_MINUTES", "10") or "10")
        except Exception:
            max_age_minutes = 10
        if max_age_minutes <= 0:
            max_age_minutes = 10
        cutoff_dt = _now_local().replace(tzinfo=None) - timedelta(minutes=max_age_minutes)
        ...
        for r in rows:
            ...
            if msg.startswith("🚨"):
                ...
                payload = f"🚨 แจ้งเตือนใหม่\n{msg}"
            _line_push_to_all_linked(payload)

        _set_state(cur, "last_alert_push_id", str(max_id))
        conn.commit()
    except Exception as e:
        app.logger.error(f"[LINE PUSH JOB] Error: {e}", exc_info=True)
    finally:
        try:
            cur.close()
        finally:
...
def start_scheduler():
    """Legacy in-app scheduler. Prefer worker.py in Docker deployments."""
    global _scheduler

    if os.environ.get("DISABLE_IN_APP_SCHEDULER", "0").strip().lower() in ("1", "true", "yes", "y", "on"):
        app.logger.info("[SCHEDULER] Disabled by DISABLE_IN_APP_SCHEDULER")
        return
    ...
    # Poll new alerts and push to LINE every 60 seconds
    _scheduler.add_job(
        _run_line_alert_push_job,
        trigger=IntervalTrigger(seconds=60),
        id="line_alert_push_60s",
        replace_existing=True,
        max_instances=1,
    )
    _scheduler.start()
...
if __name__ == "__main__":
    ...
    debug = os.environ.get("FLASK_DEBUG", "0").lower() in ("1", "true", "yes", "y", "on")
